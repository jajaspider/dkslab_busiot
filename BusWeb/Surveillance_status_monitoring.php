<?php
  ...
  if(mysqli_connect_errno($conn)){
    ...
    mysqli_select_db($conn, "surveillance_status");
    ...
    $lo1 = array('매우좋음', '좋음', '보통', '나쁨', '매우나쁨');
    $randlo1 = rand(0,count($lo1)-1);


    $sql = "INSERT INTO status_tb (bus_num, plate, status , times)
...
<html lang="ko">
  ...
  <body>
    ...
    <div class="container-fluid">
     <div class="row">
      <h2>차량 상태 모니터링</h2>
       <div id="piechart" style="width: 800px; height: 600px; margin: auto;">
         <?php

           $cvar1 = "SELECT COUNT(*) FROM status_tb where status = '매우좋음'";
           $cvar2 = "SELECT COUNT(*) FROM status_tb where status = '좋음'";
           $cvar3 = "SELECT COUNT(*) FROM status_tb where status = '보통'";
           $cvar4 = "SELECT COUNT(*) FROM status_tb where statuS = '나쁨'";
           $cvar5 = "SELECT COUNT(*) FROM status_tb where status = '매우나쁨'";
       ...
       <script type="text/javascript">
         ...
         function drawChart() {
           ...
           var data = google.visualization.arrayToDataTable([
             ['Task', 'Hours per Day'],
             ['매우좋음',     <?php $res = mysqli_query($conn, $cvar1);
                                   while($row = mysqli_fetch_array($res)){
                                     echo $row[0];
                                   }
                                   ?>
             ],
             ['좋음',     <?php $res = mysqli_query($conn, $cvar2);
                                   while($row = mysqli_fetch_array($res)){
                                     echo $row[0];
                                   }
                                   ?>
             ],
             ['보통',     <?php $res = mysqli_query($conn, $cvar3);
                                   while($row = mysqli_fetch_array($res)){
                                     echo $row[0];
                                   }
                                   ?>
             ],
             ['나쁨',     <?php $res = mysqli_query($conn, $cvar4);
                                   while($row = mysqli_fetch_array($res)){
                                     echo $row[0];
                                   }
                                   ?>
             ],
             ['매우나쁨',     <?php $res = mysqli_query($conn, $cvar5);
                                   while($row = mysqli_fetch_array($res)){
                                     echo $row[0];
                                   }
                                   ?>
