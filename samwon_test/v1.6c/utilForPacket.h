...
/**
	*@brief		각 센서 데이터를 읽어와서 변수로 저장합니다.
	*@details	각 센서 데이터를 읽어와서 _output_sensorDataValue에 덮어씌웁니다.\n
				MoveData_sensor2value에 사용된 각 함수들은 모두 sensor2value로 끝납니다.
	*@param		_output_sensorDataValue 센서 데이터 값을 덮어씌울 변수입니다. (output)
	*@param		serialId 초기화 할 시리얼ID입니다. 프로그램 실행 시 옵션으로 설정해 줄 수 있습니다.
	*@param		terminalId 초기화 할 터미널ID입니다. 프로그램 실행 시 옵션으로 설정해 줄 수 있습니다.
	*@author	정동진
	*@date		2017.07.03
*/
...
/**
	*@brief		벨류 데이터로 저장된 센서 데이터에서 패킷 데이터를 가져옵니다.
	*@details	sensorDataValue로 패킷을 만듭니다.\n
				만들어진 패킷은 _output_sensorDataPacket에 저장됩니다.\n
				MoveData_value2packet에 사용된 각 함수들은 모두 value2packet으로 끝납니다.
	*@param		sensorDataValue 패킷으로 만들 벨류 데이터입니다.
	*@param		_output_sensorDataPacket 만들어진 패킷이 저장될 변수입니다. (output)
	*@author	정동진
	*@date		2017.07.03
*/
...
/**
  ...
  *@see			MoveData_value2packet
	*@param		sensorDataValue 패킷으로 만들 데이터입니다.
	*@param		_output_sensorDataPacket 만들어진 패킷이 저장될 변수입니다. (output)
	*@author	정동진
	*@date		2017.07.03
*/
...
/**
	...
	*@details	sensorDataPacket 패킷을 _output_buff버퍼에 넣습니다.\n
				MoveData_packet2buff에 사용된 각 함수들은 모두 packet2buff로 끝납니다.
	*@param		sensorDataPacket 버퍼에 넣을 패킷 데이터입니다.
	*@param		_output_buff 패킷이 저장될이 버퍼입니다. (output)
	*@author	정동진
	*@date		2017.07.03
*/
